...
def convert2dot(behavior_tree):
    ...
    dot.attr(rankdir='TB') # TB: horizontal layout, LR: vertical layout
    root = behavior_tree
    parent_dot_name = str(hash(root))
    dot.node(parent_dot_name, root.get('ID'), shape='octagon')
...
def add_nodes(dot, parent_dot_name, parent_node):
    for node in list(parent_node):
        ...
        dot.node(str(hash(node)), label, fillcolor=node_color(node.tag), style='filled', shape=node_shape(node.tag))
...
def make_label(node):
    ...
    if id:
        tag = id
    		...
    		tag = node.tag
    label += '<tr><td align="text"><i>' + tag + '</i></td></tr>'
    name = node.get('name')
    if name:
        label += '<tr><td align="text">' + name + '</td></tr>'
    for (param_name, value) in node.items():
    		if not (param_name == 'ID' or param_name == 'name'):
...
def make_legend():
    ...
    legend.node('Unknown', shape='box', style='filled', fillcolor="grey")
    legend.node('Action', 'Action', shape='box', style='filled', fillcolor="cornflowerblue")
    legend.node('Condition', 'Condition', shape='ellipse', style='filled', fillcolor="yellow2")
    legend.node('Control', 'Control', shape='box', style='filled', fillcolor="chartreuse4")
    legend.node('Decorator', 'Decorator', shape='diamond', style='filled', fillcolor="darkorange1")
    legend.node('SubTree', 'BehaviorTree', shape='octagon', style='filled', fillcolor="white")
    return legend
